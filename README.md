...
- **注册/验证码自动重试**：hCaptcha token 被注册接口拒绝时换新 token 重提（最多 3 次）；验证码输错/过期/提交超时时自动请求新验证码重试（最多 3 次）
- **页面加载失败自恢复**：注册后跳转链中遇到 chrome-error 页时自动重新打开 build.nvidia.com 接回流程，无需重新登录
...
完成后，妥善保管本地生成的 `accounts.csv`，账号密码和 API Key 都属于敏感信息，不要提交到 GitHub；
